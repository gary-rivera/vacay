function earliestBloomingDay(plantTime: number[], growTime: number[]): number {
    let n = plantTime.length;
    let seeds: number[][] = new Array(n).fill(0).map(() => new Array(2).fill(0));
    ...
    for (let seed of seeds) {
        currentDay += seed[0];
        maxDay = Math.max(maxDay, currentDay + seed[1]);
    }
