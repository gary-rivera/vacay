...
class CBTInserter {
    ...
    constructor(root: TreeNode) {
        ...
        for(let i = 0; i < this.nodes.length; ++i) {
            if(this.nodes[i].left != null) this.nodes.push(this.nodes[i].left);
            if(this.nodes[i].right != null) this.nodes.push(this.nodes[i].right);
        }
    }
    ...
    insert(v: number): number {
        ...
        if(N % 2 == 1)
            this.nodes[(N-1) / 2].left = node;
        else
            this.nodes[(N-1) / 2].right = node;
        return this.nodes[(N-1) / 2].val;
    }
