...
function dieSimulator(n: number, rollMax: number[]): number {
    let dp = Array.from({ length: n + 1 }, () => Array.from({ length: 7 }, () => Array(16).fill(0)));
    dp[0][6] = 1;
    let sum = Array.from({ length: n + 1 }, () => Array(7).fill(0));
    sum[0][6] = 1;

    for (let i = 1; i <= n; i++) {
        for (let j = 0; j < 6; j++) {
            for (let k = 1; k <= rollMax[j] && k <= i; k++) {
                dp[i][j] = (dp[i][j] + sum[i - k][6] - sum[i - k][j] + MOD) % MOD;
            }
        }
        for (let j = 0; j < 6; j++) {
            sum[i][j] = (sum[i - 1][j] + dp[i][j]) % MOD;
        }
        sum[i][6] = (sum[i][6] + sum[i][5]) % MOD;
    }
