function greatestLetter(s: string): string {
    ...
    for(let i = 0; i < s.length; i++) {
        const lowerCaseExists = s.includes(s[i].toLowerCase());
        const upperCaseExists = s.includes(s[i].toUpperCase());
        if(lowerCaseExists && upperCaseExists && s[i].toUpperCase() > maxChar) {
            maxChar = s[i].toUpperCase();
        }
    }
    return maxChar;
}
