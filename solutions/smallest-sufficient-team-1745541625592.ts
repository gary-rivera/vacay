function smallestSufficientTeam(req_skills: string[], people: string[][]): number[] {
    ...
    const dp = new Array(1 << n).fill(0).map(() => new Array(m + 1).fill(Number.MAX_SAFE_INTEGER));
    const parent = new Array(1 << n).fill(0).map(() => new Array(m + 1).fill(-1));
    const skills = new Array(m).fill(0);
    ...
    for (let i = 0; i < m; i++) {
        for (let skill of people[i]) {
            if (skillToIndex.has(skill)) {
                skills[i] |= (1 << skillToIndex.get(skill)!);
            }
        }
    }
    ...
    for (let i = 0; i < (1 << n); i++) {
        for (let j = 0; j < m; j++) {
            if (dp[i][j] < Number.MAX_SAFE_INTEGER) {
                const next = i | skills[j];
                if (dp[next][j + 1] > dp[i][j] + 1) {
