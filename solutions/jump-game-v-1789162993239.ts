function maxJumps(arr: number[], d: number): number {
    ...
    const indices = arr.map((val, index) => index).sort((a, b) => arr[a] - arr[b]);
    let maxJump = 0;

    for (let i = 0; i < arr.length; i++) {
        const index = indices[i];
        for (let j = index - 1; j >= Math.max(0, index - d); j--) {
            if (arr[j] >= arr[index]) break;
            dp[index] = Math.max(dp[index], 1 + dp[j]);
        }
        for (let j = index + 1; j <= Math.min(arr.length - 1, index + d); j++) {
            if (arr[j] >= arr[index]) break;
            dp[index] = Math.max(dp[index], 1 + dp[j]);
        }
        maxJump = Math.max(maxJump, dp[index]);
    }
