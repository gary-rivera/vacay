...
function btreeGameWinningMove(root: TreeNode | null, n: number, x: number): boolean {
    let redLeft = 0, redRight = 0;
    countRed(root, x);
    return Math.max(Math.max(redLeft, redRight), n - redLeft - redRight - 1) > n / 2;

    function countRed(node: TreeNode | null, x: number): number {
        if (node == null) return 0;
        let left = countRed(node.left, x);
        let right = countRed(node.right, x);
        if (node.val == x) {
            redLeft = left;
            redRight = right;
        }
