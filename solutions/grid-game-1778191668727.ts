function maxPoints(grid: number[][]): number {
    let n = grid[0].length;
    let dp = Array.from({length: 2}, () => Array.from({length: n}, () => Array.from({length: 2}, () => 0)));
        ...
        for (let p1 = 0; p1 < 2; ++p1) {
            for (let p2 = 0; p2 < 2; ++p2) {
                ...
                if (p1 === p2) {
                    ...
                    points = grid[p1][c] + grid[p2][c];
                }

                if (c != n - 1) {
                    dp[p1][c][p2] = Math.max(dp[p1][c][p2], dp[p1][c + 1][p2] + points);
                    dp[p1][c][p2] = Math.max(dp[p1][c][p2], dp[1 - p1][c + 1][1 - p2] + points);
                } else {
                    dp[p1][c][p2] = points;
                }
            }
