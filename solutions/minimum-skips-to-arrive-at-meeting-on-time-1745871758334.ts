function minSkips(dist: number[], speed: number, hoursBefore: number): number {
    const n = dist.length;
    const dp = Array.from({ length: n + 1 }, () => Array(n + 1).fill(Number.MAX_SAFE_INTEGER));
    dp[0][0] = 0;
    for (let i = 1; i <= n; i++) {
        const time = Math.ceil(dist[i - 1] / speed);
        for (let j = 0; j <= i; j++) {
            if (j < i) {
                dp[i][j] = Math.min(dp[i][j], Math.ceil(dp[i - 1][j] + time));
            }
            if (j > 0) {
