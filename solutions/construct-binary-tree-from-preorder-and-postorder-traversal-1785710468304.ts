...
function constructFromPrePost(preorder: number[], postorder: number[]): TreeNode | null {
    let preIndex = 0, posIndex = 0;
    function construct(stop: number): TreeNode | null {
        if (preorder[preIndex] != stop) {
            let root = new TreeNode(preorder[preIndex++]);
            root.left = construct(preorder[preIndex]);
            root.right = construct(stop);
            posIndex++;
            return root;
        } else {
            posIndex++;
