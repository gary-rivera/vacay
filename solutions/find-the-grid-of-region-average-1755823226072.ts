function calculateGridResult(image: number[][], threshold: number): number[][] {
    const m = image.length;
    const n = image[0].length;
    const result = Array.from({ length: m }, () => Array(n).fill(0));
    const regions = Array.from({ length: m }, () => Array(n).fill(0));
    ...
    for (let i = 0; i < m; i++) {
        for (let j = 0; j < n; j++) {
            if (regions[i][j] > 0) {
                result[i][j] = Math.floor(result[i][j] / regions[i][j]);
            } else {
