type Order = [string, string, string];

function displayTable(orders: Order[]): string[][] {
    let foodSet = new Set<string>();
    let tableMap = new Map<string, Map<string, number>>();

    for (let order of orders) {
        let [_, table, food] = order;
        if (!tableMap.has(table)) {
            tableMap.set(table, new Map<string, number>());
        }
        let foodMap = tableMap.get(table)!;
        foodMap.set(food, (foodMap.get(food) || 0) + 1);
        foodSet.add(food);
    }

    let foods = Array.from(foodSet);
    foods.sort();

    let tables = Array.from(tableMap.keys());
