...
function maxActivationValue(values: number[], limits: number[]): number {
    let n = values.length;
    let activations: Activation[] = Array(n);
    for (let i = 0; i < n; i++) {
        activations[i] = { value: values[i], limit: limits[i] };
    }

    activations.sort((a, b) => a.limit - b.limit || b.value - a.value);

    let total = 0;
    let active = 0;
    let heap: number[] = [];
    for (let i = 0; i < n; i++) {
        heap.push(activations[i].value);
        heap.sort((a, b) => b - a);
        active++;

        while (active > activations[i].limit) {
            total += heap.shift() as number;
            active--;
        }
    }
