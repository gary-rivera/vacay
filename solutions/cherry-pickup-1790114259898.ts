function cherryPickup(grid: number[][]): number {
    ...
    function dfs(grid: number[][], dp: number[][][], r1: number, c1: number, c2: number): number {
        const r2 = r1 + c1 - c2;
        if (
            r1 === n ||
            r2 === n ||
            ...
            ans += Math.max(
                Math.max(dfs(grid, dp, r1, c1 + 1, c2 + 1), dfs(grid, dp, r1 + 1, c1, c2 + 1)),
                Math.max(dfs(grid, dp, r1, c1 + 1, c2), dfs(grid, dp, r1 + 1, c1, c2))
            );
            dp[r1][c1][c2] = ans;
            return ans;
        }
