function minOperations(word1: string, word2: string): number {
    ...
    for (let i = 0; i < word1Arr.length; i++) {
        if (word1Arr[i] !== word2Arr[i]) {
            let index = word1Arr.indexOf(word2Arr[i], i);
            if (index !== -1) {
                [word1Arr[i], word1Arr[index]] = [word1Arr[index], word1Arr[i]];
                operations++;
            } else {
                word1Arr[i] = word2Arr[i];
                operations++;
            }
        }
    }
...
/*
question: You are given two strings, word1 and word2, of equal length. You need to transform word1 into word2.
