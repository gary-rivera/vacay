function reverseFirstKChars(s: string, k: number): string {
    let firstKChars = s.slice(0, k).split('').reverse().join('');
    let remainingChars = s.slice(k);
    return firstKChars + remainingChars;
}
