...
function calcEquation(equations: string[][], values: number[], queries: string[][]): number[] {
    ...
    for (const [a, b] of queries) {
        if (!graph[a] || !graph[b]) {
            ...
            results.push(dfs(a, b, 1, graph, new Set()));
        }
    }
...
function dfs(current: string, target: string, product: number, graph: Graph, visited: Set<string>): number {
    ...
    for (const neighbor in graph[current]) {
        const result = dfs(neighbor, target, product * graph[current][neighbor], graph, visited);
