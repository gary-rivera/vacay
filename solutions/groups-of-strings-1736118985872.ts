function maxGroups(words: string[]): number[] {
    ...
    let maxGroup = 0;
    for (const [mask, freq] of map) {
        count[bitCount(mask)] = Math.max(count[bitCount(mask)], freq);
        maxGroup = Math.max(maxGroup, freq);
    }
    let maxGroups = 0;
    for (let i = 1; i <= 26; i++) {
        maxGroups += count[i];
    }
    return [maxGroups, maxGroup];
}
