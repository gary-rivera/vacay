function minCost(cost1: number, cost2: number, costBoth: number, need1: number, need2: number): number {
    let minCost = 0;
    if (costBoth < cost1 + cost2) {
        minCost += costBoth * Math.min(need1, need2);
        need1 -= Math.min(need1, need2);
        need2 -= Math.min(need1, need2);
    }
    minCost += cost1 * need1 + cost2 * need2;
    return minCost;
}

/*
question: You are given five integers cost1, cost2, costBoth, need1, and need2.
