...
function minMutation(start: Gene, end: Gene, bank: Gene[]): number {
    const bankSet = new Set(bank);
    if (!bankSet.has(end)) return -1;

    const queue: QueueNode[] = [{ gene: start, mutations: 0 }];
    const visited = new Set<Gene>();

    while (queue.length) {
        const { gene, mutations } = queue.shift() as QueueNode;
        ...
        for (let i = 0; i < gene.length; i++) {
            for (const nucleotide of ['A', 'C', 'G', 'T']) {
                ...
                if (!visited.has(mutated) && bankSet.has(mutated)) {
                    visited.add(mutated);
                    queue.push({ gene: mutated, mutations: mutations + 1 });
                }
            }
        }
    }
