function minLevelsAliceShouldPlay(possible: number[]): number {
    ...
    for (let i = 1; i < n; i++) {
        prefixSum[i] = prefixSum[i - 1] + (possible[i] == 1 ? 1 : -1);
    }

    suffixSum[n - 1] = possible[n - 1] == 1 ? 1 : -1;
    for (let i = n - 2; i >= 0; i--) {
        suffixSum[i] = suffixSum[i + 1] + (possible[i] == 1 ? 1 : -1);
    }
    ...
    return minLevels == n ? -1 : minLevels;
}

/*
question: You are given a binary array possible of length n.
