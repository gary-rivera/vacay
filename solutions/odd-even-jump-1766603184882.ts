function oddEvenJumps(arr: number[]): number {
    ...
    for (let i = n - 2; i >= 0; --i) {
        let v = arr[i];
        if (map.has(v)) {
            higher[i] = lower[map.get(v)];
            lower[i] = higher[map.get(v)];
        } else {
            let keys = Array.from(map.keys());
            let higherKeys = keys.filter(key => key >= v);
            let lowerKeys = keys.filter(key => key <= v);
            if (higherKeys.length > 0) {
                let minHigherKey = Math.min(...higherKeys);
                higher[i] = lower[map.get(minHigherKey)];
            }
            if (lowerKeys.length > 0) {
                let maxLowerKey = Math.max(...lowerKeys);
                lower[i] = higher[map.get(maxLowerKey)];
            }
        }
        map.set(v, i);
    }
    return higher.filter(val => val).length;
}
