class TweetCounts {
    ...
    constructor() {
        this.tweetMap = new Map<string, number[]>();
    }
    ...
    getTweetCountsPerFrequency(freq: string, tweetName: string, startTime: number, endTime: number): number[] {
        ...
        let result: number[] = new Array(Math.floor((endTime - startTime) / interval) + 1).fill(0);
        if (this.tweetMap.has(tweetName)) {
            for (let time of this.tweetMap.get(tweetName)!) {
                if (time >= startTime && time <= endTime) {
                    result[Math.floor((time - startTime) / interval)]++;
                }
            }
        }
        return result;
