function maxNumOfSubstrings(s: string): string[] {
    let first = new Array(26).fill(Number.MAX_VALUE);
    let last = new Array(26).fill(Number.MIN_VALUE);
    let res = [];
    let check = [];
    ...
    let l = 0, r = 0;
    for (let i = 0; i < s.length; i++) {
        l = Math.min(l, first[s.charCodeAt(i) - 97]);
        r = Math.max(r, last[s.charCodeAt(i) - 97]);
        if (i == r) {
