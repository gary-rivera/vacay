class CombinationIterator {
    ...
    private generateCombinations(characters: string, combinationLength: number, start: number, combination: string) {
        if (combinationLength === 0) {
            this.combinations.push(combination);
            return;
        }
        for (let i = start; i < characters.length; i++) {
            this.generateCombinations(characters, combinationLength - 1, i + 1, combination + characters[i]);
        }
    }
