...
function maxSumBST(root: TreeNode | null): number {
    ...
    function postOrder(node: TreeNode | null): [boolean, number, number, number] {
        ...
        const [isLeftBST, leftMax, leftMin, leftSum] = postOrder(node.left);
        const [isRightBST, rightMax, rightMin, rightSum] = postOrder(node.right);
        if (isLeftBST && isRightBST && node.val > leftMax && node.val < rightMin) {
            maxSum = Math.max(maxSum, leftSum + rightSum + node.val);
            return [true, Math.max(rightMax, node.val), Math.min(leftMin, node.val), leftSum + rightSum + node.val];
        } else {
            return [false, 0, 0, 0];
        }
    }
