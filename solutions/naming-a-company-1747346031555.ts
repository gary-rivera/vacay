function countValidNames(ideas: string[]): number {
    let validNames = new Set<string>();
    for (let i = 0; i < ideas.length; i++) {
        for (let j = i + 1; j < ideas.length; j++) {
            let ideaA = ideas[i][0] + ideas[j].slice(1);
            let ideaB = ideas[j][0] + ideas[i].slice(1);
            if (!ideas.includes(ideaA) && !ideas.includes(ideaB)) {
                validNames.add(ideaA + " " + ideaB);
                validNames.add(ideaB + " " + ideaA);
            }
        }
    }
