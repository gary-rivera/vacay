...
function maxScore(nums: number[], k: number): number {
    const mod = 1e9 + 7;
    const n = nums.length;
    const primes = Array(100001).fill(0);
    ...
    for (let r = 0; r < n; ++r) {
        prefix[r + 1] = (prefix[r] + primes[nums[r]]) % mod;
        for (let l = 0; l <= r; ++l) {
            pq.push([primes[nums[l]] - (prefix[r + 1] - prefix[l]), l, r]);
        }
    }
    ...
    while (k-- > 0) {
        while (true) {
            const [negGain, l, r] = pq.top();
            if (used[l] <= r && r < used[l + 1]) {
                used[l] = r + 1;
                ans = (ans * nums[l]) % mod;
                break;
            }
