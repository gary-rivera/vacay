function rankTeams(votes: string[]): string {
    ...
    let voteLength = votes[0].length;

    for (let vote of votes) {
        for (let i = 0; i < voteLength; i++) {
            if (!voteCounts[vote[i]]) {
                voteCounts[vote[i]] = new Array(voteLength).fill(0);
            }
            voteCounts[vote[i]][i]++;
        }
    }

    let teams = Object.keys(voteCounts);
    teams.sort((a, b) => {
        for (let i = 0; i < voteLength; i++) {
            if (voteCounts[a][i] > voteCounts[b][i]) {
                return -1;
            } else if (voteCounts[a][i] < voteCounts[b][i]) {
                return 1;
            }
        }
        return a < b ? -1 : 1;
