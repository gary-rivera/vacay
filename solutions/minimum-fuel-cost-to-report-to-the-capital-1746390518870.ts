...
function minFuel(roads: Road[], seats: number): number {
    const n = roads.length + 1;
    const adjList: number[][] = Array.from({ length: n }, () => []);
    const dist: number[] = Array.from({ length: n }, () => 0);
    ...
    const dfs = (node: number, parent: number, depth: number) => {
        visited[node] = true;
        dist[node] = depth;
