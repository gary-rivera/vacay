...
class Router {
    ...
    addPacket(source: number, destination: number, timestamp: number): boolean {
        ...
        if (this.packets.length === this.memoryLimit) {
            ...
            const removedPacketKey = `${removedPacket.source}-${removedPacket.destination}-${removedPacket.timestamp}`;
            this.packetSet.delete(removedPacketKey);
        }
    ...
    forwardPacket(): number[] {
        ...
        const packetKey = `${packet.source}-${packet.destination}-${packet.timestamp}`;
