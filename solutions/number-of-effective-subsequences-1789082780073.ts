function countEffectiveSubsequences(nums: number[]): number {
    const MOD = 1e9 + 7;
    const MAX = 1 << 20;
    const dp: number[] = new Array(MAX).fill(0);
    const pow: number[] = new Array(nums.length + 1).fill(0);
    pow[0] = 1;
    ...
    let res = pow[nums.length];
    for (let mask = 1; mask < MAX; mask++) {
        if (dp[mask] === nums.length) {
            res = (res - pow[nums.length - dp[mask ^ MAX]] + MOD) % MOD;
        }
    }
    return res;
}
