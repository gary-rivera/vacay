function minCostToEqualStrings(s: string, t: string, flipCost: number, swapCost: number, crossCost: number): number {
    let n = s.length;
    let cost = 0;
    let mismatchCount = 0;
    ...
    let flipAllCost = mismatchCount / 2 * flipCost * 2;
    let swapAllCost = mismatchCount / 2 * swapCost;
    let crossAllCost = mismatchCount / 2 * crossCost;

    cost += Math.min(flipAllCost, swapAllCost, crossAllCost);

    return cost;
}

/*
question: You are given two binary strings s and t, both of length n, and three positive integers flipCost, swapCost, and crossCost.
