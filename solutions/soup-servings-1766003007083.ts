class Soup {
    ...
    constructor() {
        for (let i = 0; i <= 200; i++) {
            this.dp.push(new Array(200).fill(-1));
        }
    }
    ...
    private solve(a: number, b: number): number {
        ...
        this.dp[a][b] = 0.25 * (this.solve(a - 100, b) + this.solve(a - 75, b - 25) + this.solve(a - 50, b - 50) + this.solve(a - 25, b - 75));

        return this.dp[a][b];
    }

    public soupProbability(n: number): number {
        return this.solve(n, n);
    }
}

let soup = new Soup();
console.log(soup.soupProbability(50));  // Output: 0.625
console.log(soup.soupProbability(100));  // Output: 0.71875
