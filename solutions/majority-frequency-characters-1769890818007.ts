function majorityFrequencyGroup(s: string): string {
    ...
    let groupMap: { [key: number]: string[] } = {};
    let maxGroupSize = 0;
    let maxFrequency = 0;
    ...
    for (let char in frequencyMap) {
        let frequency = frequencyMap[char];
        if (groupMap[frequency]) {
            groupMap[frequency].push(char);
        } else {
            groupMap[frequency] = [char];
        }

        // Update the maximum group size and frequency
        if (groupMap[frequency].length > maxGroupSize || (groupMap[frequency].length === maxGroupSize && frequency > maxFrequency)) {
            maxGroupSize = groupMap[frequency].length;
            maxFrequency = frequency;
        }
    }

    // Return the characters in the majority frequency group
    return groupMap[maxFrequency].join('');
}
