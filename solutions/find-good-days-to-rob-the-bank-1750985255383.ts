function goodDaysToRob(security: number[], time: number): number[] {
    let goodDays: number[] = [];
    for (let i = time; i < security.length - time; i++) {
        let before = security.slice(i - time, i);
        let after = security.slice(i + 1, i + time + 1);
        if (isNonIncreasing(before) && isNonDecreasing(after) && security[i] <= after[0] && security[i] >= before[before.length - 1]) {
            goodDays.push(i);
        }
    }
    return goodDays;
...
function isNonIncreasing(arr: number[]): boolean {
