function maxValue(n: number, index: number, maxSum: number): number {
    ...
    while (left < right) {
        ...
        if (sum(mid) > maxSum) right = mid - 1;
        else left = mid;
    }
    return left + 1;

    function sum(mid: number): number {
        let min = Math.max(0, mid - index), max = mid;
        let leftSum = (min + max) * (max - min + 1) / 2;
        min = Math.max(0, mid - ((n - 1) - index)), max = mid;
        let rightSum = (min + max) * (max - min + 1) / 2;
