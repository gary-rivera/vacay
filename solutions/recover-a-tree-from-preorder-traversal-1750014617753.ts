...
function recoverFromPreorder(traversal: string): TreeNode | null {
    ...
    function dfs(depth: number): TreeNode | null {
        let level = 0;
        while (traversal[index + level] === '-') {
            level++;
        }
        if (level !== depth) {
            return null;
        }
        let next = index + level;
        while (next < traversal.length && traversal[next] !== '-') {
            next++;
        }
        let node = new TreeNode(parseInt(traversal.slice(index + level, next)));
        index = next;
