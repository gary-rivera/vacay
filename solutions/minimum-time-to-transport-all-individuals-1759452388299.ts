function minTimeToCross(n: number, k: number, m: number, time: number[], mul: number[]): number {
    const inf = 1e9;
    let dp = Array.from({length: n + 1}, () => Array.from({length: m}, () => Array(inf)));
    dp[0][0][0] = 0;
    time.sort((a, b) => a - b);
    for (let i = 0; i < n; ++i) {
        let dp2 = Array.from({length: n + 1}, () => Array.from({length: m}, () => Array(inf)));
        for (let j = 0; j <= i; ++j) {
            for (let l = 0; l < m; ++l) {
                if (dp[i][j][l] == inf) continue;
                for (let x = 1; x <= Math.min(k, n - i); ++x) {
                    let t = time[i + x - 1] * mul[l];
                    let next = (l + Math.floor(t)) % m;
                    dp2[i + x][j][next] = Math.min(dp2[i + x][j][next], dp[i][j][l] + t);
                    if (j + x < n) {
                        let t2 = time[j] * mul[next];
                        let next2 = (next + Math.floor(t2)) % m;
                        dp2[i + x][j + x][next2] = Math.min(dp2[i + x][j + x][next2], dp[i][j][l] + t + t2);
                    }
                }
            }
        }
...
/*
question: You are given n individuals at a base camp who need to cross a river to reach a destination using a single boat. The boat can carry at most k people at a time. The trip is affected by environmental conditions that vary cyclically over m stages.
