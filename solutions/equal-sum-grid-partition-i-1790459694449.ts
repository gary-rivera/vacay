function canPartition(grid: number[][]): boolean {
    ...
    let totalRowSum = rowSums.reduce((a, b) => a + b, 0);
    let totalColSum = colSums.reduce((a, b) => a + b, 0);

    let tempRowSum = 0;
    for (let i = 0; i < m - 1; i++) {
        tempRowSum += rowSums[i];
        if (tempRowSum * 2 === totalRowSum) {
            return true;
        }
    }
    ...
    let tempColSum = 0;
    for (let j = 0; j < n - 1; j++) {
        tempColSum += colSums[j];
        if (tempColSum * 2 === totalColSum) {
            return true;
        }
    }
