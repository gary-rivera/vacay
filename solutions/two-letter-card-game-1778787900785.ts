function maxPoints(cards: string[], x: string): number {
    let points = 0;
    let map = new Map<string, number>();

    for (let card of cards) {
        if (card.includes(x)) {
            let other = card[0] === x ? card[1] : card[0];
            if (map.has(other)) {
...
/*
question: You are given a deck of cards represented by a string array cards, and each card displays two lowercase letters.
