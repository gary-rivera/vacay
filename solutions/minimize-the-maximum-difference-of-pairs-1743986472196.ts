...
function minDifference(nums: number[], p: number): number {
    nums = nums.map((num, index) => [num, index] as Pair);
    ...
    for (let [num, index] of nums) {
        while (queue.length && queue[0][1] < index - p) {
            queue.shift();
        }
        if (queue.length) {
            maxDifference = Math.max(maxDifference, num - queue[0][0]);
        }
        while (queue.length && queue[queue.length - 1][0] >= num) {
            queue.pop();
        }
