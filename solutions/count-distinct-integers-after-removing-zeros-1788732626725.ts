function countDistinctIntegers(n: number): number {
    let distinctIntegers = new Set<number>();
    for (let i = 1; i <= n; i++) {
        let integerWithoutZeros = parseInt(i.toString().replace(/0/g, ''));
        distinctIntegers.add(integerWithoutZeros);
    }
