class Solution {
    ...
    private dfs(node: number): number {
        if (this.answer[node] === -1) {
            this.answer[node] = node;
            for (let child of this.graph[node]) {
                let candidate = this.dfs(child);
                if (this.quiet[candidate] < this.quiet[this.answer[node]]) {
                    this.answer[node] = candidate;
                }
            }
        }
