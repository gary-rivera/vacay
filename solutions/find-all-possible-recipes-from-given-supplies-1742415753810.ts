function canCreateRecipes(recipes: string[], ingredients: string[][], supplies: string[]): string[] {
    ...
    while(isChanged) {
        isChanged = false;
        for(let [recipe, ingredientList] of recipeMap) {
            if(ingredientList.every(ingredient => supplySet.has(ingredient))) {
                supplySet.add(recipe);
                recipeMap.delete(recipe);
                canCreate.push(recipe);
                isChanged = true;
            }
        }
    }
