function maxScore(num: number, sum: number): string {
    if (sum > 9 * num) {
        return "";
    }
    let result: number[] = new Array(num).fill(0);
    for (let i = 0; i < num; i++) {
        let val = Math.min(9, sum);
        result[num - i - 1] = val;
        sum -= val;
    }
    return result.join("");
...
/*
question: You are given two positive integers num and sum.
