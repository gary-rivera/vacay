function maxTotalImportance(n: number, roads: number[][]): number {
    let cityConnections = new Array(n).fill(0);
    for (let road of roads) {
        cityConnections[road[0]]++;
        cityConnections[road[1]]++;
    }
    cityConnections.sort((a, b) => b - a);
    let totalImportance = 0;
    for (let i = 0; i < n; i++) {
        totalImportance += cityConnections[i] * (n - i);
    }
