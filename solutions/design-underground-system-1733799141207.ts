class UndergroundSystem {
    ...
    private journeyData: Map<string, {totalTime: number, tripCount: number}>;
    ...
    checkOut(id: number, stationName: string, t: number): void {
        ...
        if (checkInInfo) {
            ...
            const journeyInfo = this.journeyData.get(journeyKey);
            const journeyTime = t - checkInInfo.time;
            if (journeyInfo) {
                journeyInfo.totalTime += journeyTime;
                journeyInfo.tripCount += 1;
            } else {
                this.journeyData.set(journeyKey, {totalTime: journeyTime, tripCount: 1});
            }
    ...
    getAverageTime(startStation: string, endStation: string): number {
        ...
        const journeyInfo = this.journeyData.get(journeyKey);
        if (journeyInfo) {
            return journeyInfo.totalTime / journeyInfo.tripCount;
        }
        return 0;
    }
}
