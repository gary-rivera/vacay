function minPairPickup(cards: number[]): number {
    let minDistance = cards.length;
    let cardMap = new Map<number, number>();

    for (let i = 0; i < cards.length; i++) {
        if (cardMap.has(cards[i])) {
            let prevIndex = cardMap.get(cards[i]) as number;
            minDistance = Math.min(minDistance, i - prevIndex + 1);
        }
        cardMap.set(cards[i], i);
    }

    return minDistance == cards.length ? -1 : minDistance;
}
