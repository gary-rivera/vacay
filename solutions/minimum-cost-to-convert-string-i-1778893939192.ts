function minCost(source: string, target: string, original: string[], changed: string[], cost: number[]): number {
    ...
    let minCost = new Map<string, number>();
    let dp = Array(n + 1).fill(0).map(() => Array(26).fill(0));

    for (let i = 0; i < m; i++) {
        let key = original[i] + changed[i];
        if (!minCost.has(key) || cost[i] < minCost.get(key)) {
            minCost.set(key, cost[i]);
        }
    }

    for (let i = n - 1; i >= 0; i--) {
        dp[i] = [...dp[i + 1]];
        let min = dp[i + 1][target.charCodeAt(i) - 97];
        for (let j = 0; j < 26; j++) {
            let c = String.fromCharCode(97 + j);
            let key = source[i] + c;
            if (minCost.has(key)) {
                min = Math.min(min, (dp[i + 1][j] || 0) + minCost.get(key));
            }
        }
        dp[i][source.charCodeAt(i) - 97] = min;
    }

    let res = dp[0][source.charCodeAt(0) - 97];
