class Bulb {
    ...
    flipEven(): void {
        for (let i = 1; i < this.status.length; i += 2) {
            this.status[i] = !this.status[i];
        }
    }

    flipOdd(): void {
        for (let i = 0; i < this.status.length; i += 2) {
            this.status[i] = !this.status[i];
        }
    }

    flip3kPlus1(): void {
        for (let i = 0; i < this.status.length; i += 3) {
            this.status[i] = !this.status[i];
        }
    }
...
function bulbPresses(n: number, presses: number): number {
    let bulb = new Bulb(n);
    let statuses = new Set<string>();

    function dfs(presses: number): void {
        if (presses === 0) {
            statuses.add(bulb.getStatus().toString());
            return;
        }
