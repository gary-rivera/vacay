function recoverDenominations(numWays: number[]): number[] {
    let denominations: number[] = [];
    let dp: number[] = new Array(numWays.length + 1).fill(0);
    dp[0] = 1;
...
/*
question: You are given a 1-indexed integer array numWays, where numWays[i] represents the number of ways to select a total amount i using an infinite supply of some fixed coin denominations. Each denomination is a positive integer with value at most numWays.length.
