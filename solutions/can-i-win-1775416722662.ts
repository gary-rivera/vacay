function canIWin(maxChoosableInteger: number, desiredTotal: number): boolean {
    ...
    return helper(Array(maxChoosableInteger + 1).fill(false), desiredTotal, memo);
}

function helper(nums: boolean[], desiredTotal: number, memo: Map<string, boolean>): boolean {
    ...
    for (let i = 1; i < nums.length; i++) {
        if (!nums[i]) {
            nums[i] = true;
            if (!helper(nums, desiredTotal - i, memo)) {
                memo.set(key, true);
                nums[i] = false;
                return true;
            }
