function minimizeString(s: string): number {
    let arr = s.split('');
    let i = 0;
    while (i < arr.length) {
        let leftIndex = arr.lastIndexOf(arr[i], i - 1);
        let rightIndex = arr.indexOf(arr[i], i + 1);
        if (leftIndex != -1) {
...
/*
question: Given a string s, you have two types of operation:
