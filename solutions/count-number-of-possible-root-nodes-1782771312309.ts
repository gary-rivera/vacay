...
function findPossibleRoots(edges: Edge[], guesses: Edge[], k: number): number {
    const n = edges.length + 1;
    const graph: number[][] = Array.from({ length: n }, () => []);
    const guessCount: number[] = Array(n).fill(0);
    const edgeCount: number[] = Array(n).fill(0);
    const correctGuess: boolean[] = Array(n).fill(false);
    ...
    for (let i = 0; i < n; i++) {
        if ((correctGuess[i] && guessCount[i] >= k) || (!correctGuess[i] && guessCount[i] > k)) {
            possibleRoots++;
        }
    }
