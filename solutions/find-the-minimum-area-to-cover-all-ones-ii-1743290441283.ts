function minAreaRectangles(grid: number[][]): number {
    let ones = [];
    ...
    ones.sort((a, b) => a[0] - b[0] || a[1] - b[1]);

    let minArea = Infinity;
    for(let i = 0; i < ones.length; i++) {
        for(let j = i + 1; j < ones.length; j++) {
            for(let k = j + 1; k < ones.length; k++) {
                let area1 = (ones[i][0] - ones[j][0] + 1) * (ones[i][1] - ones[j][1] + 1);
                let area2 = (ones[i][0] - ones[k][0] + 1) * (ones[i][1] - ones[k][1] + 1);
                let area3 = (ones[j][0] - ones[k][0] + 1) * (ones[j][1] - ones[k][1] + 1);
