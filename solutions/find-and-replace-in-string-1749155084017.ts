function findReplaceString(s: string, indices: number[], sources: string[], targets: string[]): string {
    let replacements: {index: number, source: string, target: string}[] = [];
    ...
    for(let i = 0; i < replacements.length; i++) {
        let {index, source, target} = replacements[i];
        if(s.substr(index, source.length) === source) {
            result += s.substring(lastIdx, index) + target;
            lastIdx = index + source.length;
        }
    }
