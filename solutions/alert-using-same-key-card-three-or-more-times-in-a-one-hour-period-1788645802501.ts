type TimeLog = { name: string, time: string };

function alertNames(keyName: string[], keyTime: string[]): string[] {
    const timeLogs: TimeLog[] = keyName.map((name, i) => ({ name, time: keyTime[i] }));
    timeLogs.sort((a, b) => a.time.localeCompare(b.time));

    const alerts: Set<string> = new Set();
    const workerLogs: { [key: string]: TimeLog[] } = {};

    for (let log of timeLogs) {
        if (!workerLogs[log.name]) {
            workerLogs[log.name] = [log];
        } else {
            workerLogs[log.name].push(log);
            if (workerLogs[log.name].length >= 3) {
                const recentLogs = workerLogs[log.name].slice(-3);
                if (isWithinOneHour(recentLogs[0].time, recentLogs[2].time)) {
                    alerts.add(log.name);
                }
            }
        }
