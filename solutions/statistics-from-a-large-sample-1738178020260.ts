function sampleStats(count: number[]): number[] {
    let min = 255, max = 0, sum = 0, mode = 0, modeVal = 0, total = 0, mid1 = 0, mid2 = 0;
    for (let i = 0; i < 256; ++i) {
        if (count[i] !== 0) {
            min = Math.min(min, i);
            max = Math.max(max, i);
    ...
    let mean = sum / total;
    let mid = total / 2;
    for (let i = 0, cnt = 0; total % 2 === 0 ? i < 256 : i <= 256; ++i) {
        if (cnt < mid && cnt + count[i] >= mid) mid1 = i;
        if (cnt < mid + 1 && cnt + count[i] >= mid + 1) {
            mid2 = i;
            break;
        }
        cnt += count[i];
    }
    let median = total % 2 === 0 ? (mid1 + mid2) / 2 : mid2;
