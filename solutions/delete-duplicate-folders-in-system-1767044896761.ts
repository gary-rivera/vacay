function deleteDuplicateFolder(paths: string[][]): string[][] {
    ...
    function serialize(node) {
        ...
        for (const folder in node) {
            folders.push(folder + serialize(node[folder]));
        }
        folders.sort();
        const serial = '(' + folders.join('') + ')';
        if (count.has(serial) && count.get(serial) === 1) {
            ans.push(serial);
        }
    ...
    function deserialize(node, path) {
        for (const folder in node) {
            const newPath = path.concat(folder);
            if (count.get('(' + folder + serialize(node[folder]) + ')') === 1) {
                ans.push(newPath);
            }
            deserialize(node[folder], newPath);
        }
    }
