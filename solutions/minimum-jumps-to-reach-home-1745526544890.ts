...
function minimumJumps(forbidden: number[], a: number, b: number, x: number): number {
    ...
    const visited = new Array(MAX_POSITION).fill(0).map(() => new Array(2).fill(false));
    ...
    while (queue.length > 0) {
        const [pos, steps, back] = queue.shift() as Position;
        if (pos === x) {
            return steps;
        }
        const forward = pos + a;
        const backward = pos - b;
        if (forward < MAX_POSITION && !visited[forward][0]) {
            visited[forward][0] = true;
            queue.push([forward, steps + 1, 0]);
        }
        if (backward >= 0 && !back && !visited[backward][1]) {
            visited[backward][1] = true;
            queue.push([backward, steps + 1, 1]);
        }
    }
    return -1;
}
